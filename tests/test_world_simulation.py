...
from algohns.modules.reference_data import sp500_constituents
from algohns.modules.world_events import (
    EVENT_CATALOGUE,
    EventScope,
    expected_event_market_drift,
    sample_events,
)
from algohns.modules.world_forward import (
    TRADING_DAYS,
    ForwardConfig,
    build_world,
    simulate_world,
)
from algohns.modules.world_universe import COMPANIES, COUNTRIES, SUPPLY_LINKS
...
class TestDataset:
    ...
    def test_world_covers_major_index_names(self):
        """The globe should carry the large caps the optimiser reaches for."""
        world = {c.ticker for c in COMPANIES}
        core = {"AAPL", "MSFT", "NVDA", "AMZN", "GOOGL", "JPM", "JNJ", "XOM",
                "WMT", "UNH", "BAC", "CVX", "GE", "UNP", "AMT", "META", "DIS"}
        assert not core - world
