...
`public/world/index.html`, which is the page module 6 embeds. The JSON is
inlined rather than fetched because a single self-contained file works in
every host, including sandboxes that block same-origin data fetches.
"""
...
from algohns.modules.world_export import export_static_world  # noqa: E402

TARGET = ROOT / "public" / "world"
JSON_PATH = TARGET / "world.json"
HTML_PATH = TARGET / "index.html"
PATTERN = re.compile(r"(const WORLD = )(.*?)(;\n)", re.S)


def main() -> int:
    blob = json.dumps(export_static_world(), separators=(",", ":"))
